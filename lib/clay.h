...
#define CLAY_SIDI_LOCAL(label, index)                                          \
  Clay__HashString(label, index, Clay__GetParentElementId())
...
typedef struct {
  ...
  // When set to true, clay will hash the entire text contents of this string as
  // an identifier for its internal text measurement cache, rather than just the
  // pointer and length. This will incur significant performance cost for long
  // bodies of text.
...
typedef struct {
  void *imageData; // A transparent pointer used to pass image data through to
                   // the renderer.
  Clay_Dimensions sourceDimensions; // The original dimensions of the source
                                    // image, used to control aspect ratio.
...
typedef struct {
  bool horizontal; // Clip overflowing elements on the X axis and allow
                   // scrolling left and right.
  bool vertical; // Clip overflowing elements on the YU axis and allow scrolling
                 // up and down.
...
typedef struct {
  ...
  // Controls whether an element should clip its contents and allow scrolling
  // rather than expanding to contain them.
...
// Resets Clay's internal text measurement cache, useful if memory to represent
// strings is being re-used. Similar behaviour can be achieved on an individual
// text element level by using Clay_TextElementConfig.hashStringContents
...
CLAY_DLL_EXPORT Clay_ElementId Clay__HashString(Clay_String key,
...
typedef CLAY_PACKED_ENUM{
    ...
    CLAY__ELEMENT_CONFIG_TYPE_FLOATING, CLAY__ELEMENT_CONFIG_TYPE_SCROLL,
    CLAY__ELEMENT_CONFIG_TYPE_IMAGE,    CLAY__ELEMENT_CONFIG_TYPE_TEXT,
    CLAY__ELEMENT_CONFIG_TYPE_CUSTOM,   CLAY__ELEMENT_CONFIG_TYPE_SHARED,
...
Clay_Context *Clay__Context_Allocate_Arena(Clay_Arena *arena) {
  ...
  // Make sure the memory address passed in for clay to use is cache line
  // aligned
...
Clay__StoreScrollElementConfig(Clay_ScrollElementConfig config) {
  return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
             ? &Clay_ScrollElementConfig_DEFAULT
             : Clay__ScrollElementConfigArray_Add(
                   &Clay_GetCurrentContext()->scrollElementConfigs, config);
}
Clay_BorderElementConfig *
...
void Clay__UpdateAspectRatioBox(Clay_LayoutElement *layoutElement) {
  for (int32_t j = 0; j < layoutElement->elementConfigs.length; j++) {
    ...
    if (config->type == CLAY__ELEMENT_CONFIG_TYPE_IMAGE) {
      ...
      if (imageConfig->sourceDimensions.width == 0 ||
          imageConfig->sourceDimensions.height == 0) {
        break;
      }
      float aspect = imageConfig->sourceDimensions.width /
      ...
      if (layoutElement->dimensions.width == 0 &&
        ...
        layoutElement->dimensions.width =
            layoutElement->dimensions.height * aspect;
      } else if (layoutElement->dimensions.width != 0 &&
                 layoutElement->dimensions.height == 0) {
        layoutElement->dimensions.height =
            layoutElement->dimensions.height * (1 / aspect);
      }
      break;
    }
  }
...
void Clay__CloseElement(void) {
  ...
  for (int32_t i = 0; i < openLayoutElement->elementConfigs.length; i++) {
    ...
    if (config->type == CLAY__ELEMENT_CONFIG_TYPE_SCROLL) {
      elementHasScrollHorizontal =
          config->config.scrollElementConfig->horizontal;
  ...
  if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
    ...
    for (int32_t i = 0;
         i < openLayoutElement->childrenOrTextContent.children.length; i++) {
      ...
      openLayoutElement->dimensions.height =
          CLAY__MAX(openLayoutElement->dimensions.height,
                    child->dimensions.height + layoutConfig->padding.top +
      ...
      if (!elementHasScrollVertical) {
        openLayoutElement->minDimensions.height =
            CLAY__MAX(openLayoutElement->minDimensions.height,
                      child->minDimensions.height + layoutConfig->padding.top +
                          layoutConfig->padding.bottom);
      }
    ...
    for (int32_t i = 0;
      ...
      openLayoutElement->dimensions.width =
          CLAY__MAX(openLayoutElement->dimensions.width,
                    child->dimensions.width + layoutConfig->padding.left +
      ...
      if (!elementHasScrollHorizontal) {
        openLayoutElement->minDimensions.width =
            CLAY__MAX(openLayoutElement->minDimensions.width,
                      child->minDimensions.width + layoutConfig->padding.left +
                          layoutConfig->padding.right);
      }
      Clay__int32_tArray_Add(&context->layoutElementChildren, childIndex);
    }
...
void Clay__ConfigureOpenElement(const Clay_ElementDeclaration declaration) {
  ...
  if (!Clay__MemCmp((char *)(&declaration.cornerRadius),
    ...
    if (sharedConfig) {
      ...
      sharedConfig = Clay__StoreSharedElementConfig(CLAY__INIT(
          Clay_SharedElementConfig){.cornerRadius = declaration.cornerRadius});
      Clay__AttachElementConfig(
          CLAY__INIT(Clay_ElementConfigUnion){.sharedElementConfig =
                                                  sharedConfig},
          CLAY__ELEMENT_CONFIG_TYPE_SHARED);
  ...
  if (declaration.image.imageData) {
    ...
    Clay__int32_tArray_Add(&context->imageElementPointers,
                           context->layoutElements.length - 1);
  }
  if (declaration.floating.attachTo != CLAY_ATTACH_TO_NONE) {
    ...
    if (hierarchicalParent) {
      ...
      if (!openLayoutElementId.id) {
        openLayoutElementId =
            Clay__HashString(CLAY_STRING("Clay__FloatingContainer"),
  ...
  if (declaration.scroll.horizontal | declaration.scroll.vertical) {
    Clay__AttachElementConfig(
        CLAY__INIT(Clay_ElementConfigUnion){
            .scrollElementConfig =
                Clay__StoreScrollElementConfig(declaration.scroll)},
...
void Clay__InitializeEphemeralMemory(Clay_Context *context) {
  ...
  context->scrollElementConfigs =
      Clay__ScrollElementConfigArray_Allocate_Arena(maxElementCount, arena);
...
void Clay__InitializePersistentMemory(Clay_Context *context) {
  ...
  context->pointerOverIds =
      Clay__ElementIdArray_Allocate_Arena(maxElementCount, arena);
  context->debugElementData =
    ...
    for (int32_t i = 0; i < bfsBuffer.length; ++i) {
      ...
      if (sizingAlongAxis) {
        ...
        for (int32_t childOffset = 0;
          ...
          if (Clay__ElementHasConfig(parent,
                                     CLAY__ELEMENT_CONFIG_TYPE_SCROLL)) {
            Clay_ScrollElementConfig *scrollElementConfig =
                Clay__FindElementConfigWithType(
                    parent, CLAY__ELEMENT_CONFIG_TYPE_SCROLL)
                    .scrollElementConfig;
            if (((xAxis && scrollElementConfig->horizontal) ||
                 (!xAxis && scrollElementConfig->vertical))) {
          ...
          if (childSizing.type == CLAY__SIZING_TYPE_FIT) {
            *childSize = CLAY__MAX(childSizing.size.minMax.min,
...
void Clay__CalculateFinalLayout(void) {
  ...
  for (int32_t textElementIndex = 0;
       textElementIndex < context->textElementData.length; ++textElementIndex) {
    ...
    while (wordIndex != -1) {
      ...
      // measuredWord->length == 0 means a newline character
        ...
        bool finalCharIsSpace =
            textElementData->text
                .chars[lineStartOffset + lineLengthChars - 1] == ' ';
        Clay__WrappedTextLineArray_Add(
            &context->wrappedTextLines,
            CLAY__INIT(Clay__WrappedTextLine){
  ...
  for (int32_t i = 0; i < context->imageElementPointers.length; ++i) {
    Clay_LayoutElement *imageElement = Clay_LayoutElementArray_Get(
        &context->layoutElements,
  ...
  while (dfsBuffer.length > 0) {
    ...
    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
      ...
      // layout axis
      for (int32_t j = 0;
           j < currentElement->childrenOrTextContent.children.length; ++j) {
        Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(
            &context->layoutElements,
      ...
      if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
          ...
          for (int32_t i = 0; i < sortMax; ++i) {
            ...
            Clay__ElementConfigType nextType =
                Clay__ElementConfigArraySlice_Get(
                    ...
                    ->type;
            if (nextType == CLAY__ELEMENT_CONFIG_TYPE_SCROLL ||
        ...
        for (int32_t elementConfigIndex = 0;
          ...
          case CLAY__ELEMENT_CONFIG_TYPE_SCROLL: {
            ...
            renderCommand.renderData = CLAY__INIT(Clay_RenderData){
                .scroll = {
                    .horizontal =
                        elementConfig->config.scrollElementConfig->horizontal,
                    .vertical =
                        elementConfig->config.scrollElementConfig->vertical,
                }};
            break;
          }
                ...
                .image = {
                    ...
                    .sourceDimensions =
                        elementConfig->config.imageElementConfig
                            ->sourceDimensions,
                    .imageData =
                        elementConfig->config.imageElementConfig->imageData,
                }};
            emitRectangle = false;
            break;
          }
          case CLAY__ELEMENT_CONFIG_TYPE_TEXT: {
            ...
            for (int32_t lineIndex = 0;
                 lineIndex < currentElement->childrenOrTextContent
                                 .textElementData->wrappedLines.length;
                 ++lineIndex) {
                      ...
                      {.text =
                           {
                               ...
                               .letterSpacing =
                                   textElementConfig->letterSpacing,
          ...
          case CLAY__ELEMENT_CONFIG_TYPE_CUSTOM: {
            ...
            renderCommand.renderData = CLAY__INIT(Clay_RenderData){
                .custom = {
                    ...
                    .cornerRadius = sharedConfig->cornerRadius,
                    .customData =
                        elementConfig->config.customElementConfig->customData,
                }};
            emitRectangle = false;
            break;
          }
        ...
        if (!Clay__ElementHasConfig(currentElementTreeNode->layoutElement,
          ...
          if (scrollContainerData) {
            scrollContainerData->contentSize = CLAY__INIT(Clay_Dimensions){
                contentSize.width + (float)(layoutConfig->padding.left +
                                            layoutConfig->padding.right),
                contentSize.height + (float)(layoutConfig->padding.top +
                                             layoutConfig->padding.bottom)};
          }
        }
      } else {
        ...
        if (Clay__ElementHasConfig(currentElement,
          ...
          if (!Clay__ElementIsOffscreen(&currentElementBoundingBox)) {
            Clay_SharedElementConfig *sharedConfig =
                Clay__ElementHasConfig(currentElement,
                                       CLAY__ELEMENT_CONFIG_TYPE_SHARED)
                    ? Clay__FindElementConfigWithType(
            ...
            Clay_BorderElementConfig *borderConfig =
                Clay__FindElementConfigWithType(
                    currentElement, CLAY__ELEMENT_CONFIG_TYPE_BORDER)
                    .borderElementConfig;
            Clay_RenderCommand renderCommand = {
                .boundingBox = currentElementBoundingBox,
        ...
        for (int32_t i = 0;
          ...
          dfsBuffer.internalArray[newNodeIndex] =
              CLAY__INIT(Clay__LayoutElementTreeNode){
                  ...
                  .nextChildOffset =
                      {.x = (float)childElement->layoutConfig->padding.left,
...
Clay__RenderDebugLayoutElementsList(int32_t initialRootsLength,
  ...
  for (int32_t rootIndex = 0; rootIndex < initialRootsLength; ++rootIndex) {
    ...
    while (dfsBuffer.length > 0) {
      ...
      CLAY({.id = CLAY_IDI("Clay__DebugView_ElementOuter", currentElement->id),
      ...
      if (Clay__ElementHasConfig(currentElement,
        ...
        Clay_TextElementConfig *rawTextConfig =
            offscreen ? CLAY_TEXT_CONFIG({.textColor = CLAY__DEBUGVIEW_COLOR_3,
                                          .fontSize = 16})
                      : &Clay__DebugView_TextNameConfig;
        CLAY({.layout = {.sizing = {.height = CLAY_SIZING_FIXED(
  ...
  if (context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME) {
    ...
    for (int32_t i = (int)context->pointerOverIds.length - 1; i >= 0; i--) {
      Clay_ElementId *elementId =
          Clay__ElementIdArray_Get(&context->pointerOverIds, i);
...
void Clay__RenderDebugView(void) {
  ...
  CLAY({.id = CLAY_ID("Clay__DebugView"),
    ...
    CLAY({.id = scrollId,
      ...
      CLAY({.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)},
        ...
        float contentWidth = Clay__GetHashMapItem(panelContentsId.id)
                                 ->layoutElement->dimensions.width;
        CLAY({.layout = {.sizing = {.width = CLAY_SIZING_FIXED(contentWidth)},
                         .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {}
    ...
    if (context->debugSelectedElementId != 0) {
      ...
      CLAY({.layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(300)},
                       .layoutDirection = CLAY_TOP_TO_BOTTOM},
            .backgroundColor = CLAY__DEBUGVIEW_COLOR_2,
        ...
        for (int32_t elementConfigIndex = 0;
          ...
          case CLAY__ELEMENT_CONFIG_TYPE_IMAGE: {
            Clay_ImageElementConfig *imageConfig =
                elementConfig->config.imageElementConfig;
            CLAY({.id = CLAY_ID("Clay__DebugViewElementInfoImageBody"),
              ...
              CLAY({.id =
                        CLAY_ID("Clay__DebugViewElementInfoImageDimensions")}) {
                CLAY_TEXT(CLAY_STRING("{ width: "), infoTextConfig);
                CLAY_TEXT(
              ...
              CLAY({.layout = {.sizing = {.width = CLAY_SIZING_GROW(
                                              0, imageConfig->sourceDimensions
                                                     .width)}},
                    .image = *imageConfig}) {}
            }
            break;
          ...
          case CLAY__ELEMENT_CONFIG_TYPE_BORDER: {
            ...
            CLAY({.id = CLAY_ID("Clay__DebugViewElementInfoBorderBody"),
                  .layout = {.padding = attributeConfigPadding,
                             .childGap = 8,
                             .layoutDirection = CLAY_TOP_TO_BOTTOM}}) {
  ...
  for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1;
    ...
    while (dfsBuffer.length > 0) {
      ...
      int32_t clipElementId = Clay__int32_tArray_GetValue(
          &context->layoutElementClipElementIds,
          currentElement - context->layoutElements.internalArray);
      ...
      if (mapItem) {
        if ((Clay__PointIsInsideRect(position, elementBox)) &&
          ...
          Clay__ElementIdArray_Add(&context->pointerOverIds,
                                   mapItem->elementId);
          found = true;

          if (mapItem->idAlias != 0) {
            Clay__ElementIdArray_Add(
                &context->pointerOverIds,
  ...
  for (int32_t i = 0; i < context->scrollContainerDatas.length; i++) {
    ...
    for (int32_t j = 0; j < context->pointerOverIds.length;
      ...
      if (scrollData->layoutElement->id ==
          Clay__ElementIdArray_Get(&context->pointerOverIds, j)->id) {
...
Clay_RenderCommandArray Clay_EndLayout(void) {
  ...
  if (context->booleanWarnings.maxElementsExceeded) {
    ...
    if (!elementsExceededBeforeDebugView) {
      message = CLAY_STRING(
          "Clay Error: Layout elements exceeded Clay__maxElementCount after "
          "adding the debug-view to the layout.");
    } else {
      message = CLAY_STRING(
...
bool Clay_Hovered(void) {
  ...
  for (int32_t i = 0; i < context->pointerOverIds.length; ++i) {
    if (Clay__ElementIdArray_Get(&context->pointerOverIds, i)->id ==
