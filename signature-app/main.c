...
uint8_t verify(Context *ctx) {
  return verify_pdf_signature(ctx->pdf_file, ctx->verify_key_file);
}
...
void handleConfirmSignButtonInteraction(Clay_ElementId id, Clay_PointerData pointer_info, intptr_t user_data) {
  ...
  if (ctx->pin_data.pin[0] == 0) return;
...
int main() {
  ...
  Context ctx = {.mode = MODE_INITIAL, .sign_key_file = {}, .verify_key_file = {},
                 .pdf_file = {}, .sign_status = NONE, .verify_status = NONE};
